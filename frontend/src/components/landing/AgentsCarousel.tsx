...
export default function AgentsCarousel() {
  ...
  const agents = [
    {
      ...
      subtitle: 'PyMuPDF Structural Extractor',
      description: 'Parses uploaded resumes using PyMuPDF and Regex intelligence. Automatically extracts contact info, technical skills, education, experience, certifications, and volunteer work.',
      stat: '99.8%',
  ...
  return (
    <section id="agents" className="relative py-14 md:py-16 bg-[#0e0f14] border-b border-white/20 overflow-hidden z-10">
      {/* Background glow orbs */}
      <div className="absolute top-[20%] left-[-15%] w-[400px] h-[400px] rounded-full bg-purple-900/20 blur-[120px] pointer-events-none"></div>
      <div className="absolute bottom-[20%] right-[-15%] w-[400px] h-[400px] rounded-full bg-amber-500/20 blur-[120px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 text-center">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 text-left">
          <div>
            <span className="text-xs md:text-sm font-display font-black text-amber-400 uppercase tracking-widest block mb-1.5">
              SPECIALIZED AI AGENTS
            </span>
            <h2 className="text-2xl md:text-4xl font-display font-black text-white leading-tight">
              One coordinate thread. <br />
              Five specialized agents in sync.
            </h2>
          </div>

          {/* Buttons */}
          <div className="flex items-center gap-2.5 mt-5 md:mt-0">
            <button
              onClick={handlePrev}
              className="w-10 h-10 rounded-full border border-white/20 hover:border-amber-400 hover:bg-amber-500/10 text-gray-200 hover:text-white flex items-center justify-center transition-all duration-300 shadow-md"
            >
        ...
        <div className="relative min-h-[410px] py-2 flex items-center justify-center max-w-4xl mx-auto overflow-visible">
          {agents.map((agent, index) => {
            ...
            if (isActive) {
              transformClass = 'translate-x-0 scale-100 opacity-100 z-30 pointer-events-auto shadow-[0_0_35px_rgba(245,158,11,0.2)] border-amber-500/60'
            } else if (isLeft) {
              transformClass = '-translate-x-[40%] sm:-translate-x-[55%] scale-90 opacity-40 z-20 pointer-events-auto cursor-pointer hover:opacity-60'
            } else if (isRight) {
              transformClass = 'translate-x-[40%] sm:translate-x-[55%] scale-90 opacity-40 z-20 pointer-events-auto cursor-pointer hover:opacity-60'
            }
            ...
            return (
              ...
              >
                <Card className="border border-white/20 bg-neutral-900/95 backdrop-blur-2xl p-5 select-none shadow-2xl">
                  <CardHeader className="flex flex-row items-center justify-between border-b border-white/10 pb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <CardTitle className="text-base font-extrabold text-white leading-snug">{agent.title}</CardTitle>
                        <span className="text-[11px] font-display font-black text-amber-400 uppercase tracking-wider block">
                          {agent.subtitle}
                        </span>
                      </div>
                    </div>
                    <span className="text-[11px] font-display font-black text-gray-400 tracking-wider shrink-0">
                      {agent.step}
                    </span>
                  </CardHeader>
                  
                  <CardContent className="py-4 text-left flex-1 flex flex-col justify-center min-h-[110px]">
                    <p className="text-xs md:text-sm font-display font-medium text-gray-200 leading-relaxed">
                      {agent.description}
                    </p>
                  </CardContent>

                  <CardFooter className="border-t border-white/10 pt-3.5 flex items-center justify-between">
                    <div className="text-left">
                      <div className="text-xl font-display font-black text-white">{agent.stat}</div>
                      <div className="text-[9px] font-display font-bold text-gray-400 uppercase tracking-wider">
                        {agent.statLabel}
                      </div>
                    </div>
                    <button className="px-4 py-2 rounded-xl border border-white/20 hover:border-amber-400 hover:bg-amber-500/10 text-[11px] font-display font-bold tracking-wider text-gray-200 hover:text-white transition-all uppercase shadow-sm">
                      Launch Agent
                    </button>
                  </CardFooter>
                </Card>
          ...
          {agents.map((_, index) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                index === activeIndex ? 'w-8 bg-amber-400' : 'w-2.5 bg-neutral-700 hover:bg-neutral-600'
              }`}
            ></button>
          ))}
        </div>
